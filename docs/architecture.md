...
| Worker Handoff | `extensions/worker-handoff.ts` | Reviewed-deliverable provenance, model/thinking choice shaping, and human-confirmation text. |
...
- **Handoff stays human-started and transcript-free.** Approval changes no context. Use → Parent queues a chip only. Use → Worker writes reviewed sidecar bytes, forces fresh launch, and records provenance; selected model/thinking override kind defaults internally without adding public spawn flags.
...
- **Attach means switch when already inside tmux.** `/docket attach` deliberately uses `switch-client` from inside tmux and a copyable `attach` command outside tmux. Workers record their direct parent tmux target at spawn so `/docket attach parent` can return from a worker to its parent. Docket does not pretend tmux navigation is session evidence.
