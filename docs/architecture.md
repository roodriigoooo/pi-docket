...
| Worker Overlap | `extensions/worker-overlap.ts` | Pure grading of that overlap: old-file hunk ranges parsed from frozen patches, `same-file`/`adjacent`/`contested`, confirmation body and summary line. No git, no fs, no tmux — the observed apply check lives in Worker Changes. |
