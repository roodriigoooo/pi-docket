...
| Worker Conflicts | `extensions/worker-conflicts.ts` | Edited-file overlap detection across workers; the cheap path-level warning the dock and dashboard render every pass. Paths are compared in the project's coordinate space: a worker's own workspace prefix is stripped, so a live absolute `edit` path still meets a peer's repo-relative frozen change set. |
| Worker Overlap | `extensions/worker-overlap.ts` | Pure grading of that overlap: old-file hunk ranges parsed from frozen patches, `same-file`/`adjacent`/`contested`, confirmation body, card line, and the attributed two-sided patch the overlap view reads. No git, no fs, no tmux — the observed apply check lives in Worker Changes. |
| Worker Reconcile | `extensions/worker-reconcile.ts` | Merging two workers' frozen change sets over the base they share. Pure: conflict-marker relabelling, `merge-tree` output parsing, summary shaping, the leftover-marker gate, and every surface string. Adapter: scratch-index post-image trees, `git merge-tree --write-tree` under `diff3`, and the reconciled patch. Writes no worktree, no index, and no file until a promotion applies the result. |
...
| Worker Journal | `extensions/worker-journal.ts` | The project journal: entry types, append, wholesale render of the worker-facing view, retention, and stale-base derivation. |
| Worker Bulletin | `extensions/worker-bulletin.ts` | The legacy bulletin parser, kept only so an existing one migrates into the journal once. |
...
5. `Background Work` projects the snapshot into a synthetic status artifact. Navigator ranks it alongside file edits and errors. The dock renders one row per *unsettled* worker in fixed columns sized once per render (`dockColumns`), plus an event sub-line when thinking. Settled workers — `reviewed`, or `stopped` with no claim and no untaken message — are partitioned out (`partitionDockRows`) and stand as one line; the `f8` dashboard holds them behind `tab`. Passive warnings use the same data: `silent Nm` for no recent tool/todo events, `waiting Nm` for an old parent question. A recorded verdict outranks both the overlap cell and the stale-base sub-line, since neither can change a decision already made.
...
| `bulletins/<project>.ndjson` | Journal | Append-only standing notes and landed promotions. The source of truth for staleness. |
| `bulletins/<project>.md` | Journal | Worker-facing view, regenerated in full from the ndjson on every append. Never read back as truth. |
...
- **Worker overlap is surfaced and graded, not prevented.** Isolated worktrees keep workers from clobbering each other while they work. Path overlap is detected on every render, because that is all the dock can afford. The grade — do the changed line ranges actually meet — is computed once, when the human opens a promotion, from patches already frozen on disk, and where both sides have one git is asked whether the other's patch survives instead of it being inferred. Silence is earned by observed separation; an ungradeable overlap still asks. Both sides are readable from the verdict card (`o`) and from the confirmation itself, as one patch of the contested paths with each section attributed to a worker and its task. Its exits are a revision request to one worker through the ordinary `tell` channel, or a reconciliation. The parent remains the mediator.
- **Reconciliation computes the mechanical part and hands over the residue.** Two change sets over one base are a three-way merge, so the question a collision poses — what should the file be — has a machine answer for every line the two did not both touch, and a human one for the lines they did. Docket builds each side's post-image in a scratch index, commits both as children of the shared base, and runs `git merge-tree --write-tree` under `diff3`: no worktree, no checkout, no working-copy write, and every object unreferenced. The base is chosen by proof rather than by ancestry — each candidate head is accepted only if *both* patches apply to it. Conflict markers are relabelled positionally, so every one names a worker and its task instead of an object id, and the human resolves them in their own editor. A file handed back with a marker still in it is never promoted; the gate re-reads what is actually about to be applied. The reconciled patch is an ordinary frozen patch by the time git sees it, so it goes through the one promote path and passes every guard an ordinary promotion passes. A merge is an observation, like `apply --check`; Docket never decides a reconciliation, and an old git or an unreconstructable base is a stated reason, never a quiet promotion of one side.
- **A promotion with two authors credits both of them.** The journal entry carries `workerIds`, so no contributor derives `base moved` from work that is inside the promotion, and the entry says in words that neither worker needs to redo anything — the opposite of what an ordinary promotion tells the workers it invalidates. The peer's deliverable is closed out with the `reconcile` verb: terminal, so its row settles and its generation is safe to prune, and never approval, because approval means a human judged that deliverable on its own.
...
- **Uncertainty degrades to the journal.** When nothing scores as affected, the primary action becomes posting a standing note rather than presenting an empty grid. The journal lives under the agent directory, not the repo, so every worktree reads the same current file, and `task.md` points at it by absolute path only once one exists.
...
- **Liveness is observed separately from lifecycle state.** What the work amounts to and whether a process is still there are two facts. `exitedAt` is written by the side that saw the worker leave and never overwrites what the worker reported; `workerLiveness` folds it together with terminal states, a harvested pane, and heartbeat age, and absence of proof is `unknown` rather than `live`. Only `docket_done` earns `ready`; a process that merely stopped derives `stopped`. Rendered as a hollow dot, so a worker can be ready to review and unreachable at the same time without either claim being dropped.
- **A promotion propagates without being addressed to anyone.** It is the one class of worker output that already carries the human's signature — that is what promotion means — so forwarding it crosses neither the model-context boundary nor the reviewed-premise one, and needs no confirmation and no tokens. Promotion appends a `promoted` entry naming its paths; every worker whose evidence or approved plan names one derives `base moved`. Rationale: [ADR-0008](./adr/0008-worker-messaging.md).
- **Staleness is derived, never pushed.** `deriveStaleBase` routes through the same scorer a broadcast uses, because a second notion of "affected" that scored slightly better would be a worse outcome than a shared one that sometimes scores worse. Only `affected` counts: task-text overlap is a fine reason to propose a recipient to a human and a poor reason to tell a worker its ground has shifted. Nothing wakes a worker — it reads the journal at a gate it was already going to stop at, and its guardrails state that its isolated workspace still holds the pre-promotion bytes.
- **The journal's markdown is a view, not a source.** It is regenerated in full from the ndjson on every append and no code path reads it back as truth, so the two cannot drift and hand-editing it survives exactly one append. Nothing writes the legacy bulletin format any more: a second writer would produce a file the migration would import again on the next upgrade.
