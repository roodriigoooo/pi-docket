...
- **Worker overlap is surfaced and graded, not prevented.** Isolated worktrees keep workers from clobbering each other while they work. Path overlap is detected on every render, because that is all the dock can afford. The grade — do the changed line ranges actually meet — is computed once, when the human opens a promotion, from patches already frozen on disk, and where both sides have one git is asked whether the other's patch survives instead of it being inferred. Silence is earned by observed separation; an ungradeable overlap still asks. Both sides are readable from the verdict card (`o`) and from the confirmation itself, as one patch of the contested paths with each section attributed to a worker and its task. Its exit is a revision request to one worker through the ordinary `tell` channel — no verdict, no lifecycle change, no merge. The parent remains the mediator.
