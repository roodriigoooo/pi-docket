...
- **a collision no longer costs one worker's work**: the overlap view asked *who yields*, and every answer was lossy — promote one worker and the other is left building on a base that moved, or send it back to redo work it had already finished. Both workers branched from one base, so their two change sets are a three-way merge and git computes it exactly. `Combine` is that merge, and it turns "which worker wins" into "what should the file be".
- **most collisions turn out to have no decision in them at all**: where the two sides do not meet, git merges both and the row says `merges clean, 2 files` before you commit to it. Where they do, it says `2 conflicts to resolve in 1 file`, so neither outcome is discovered halfway through an editor session.
- **the residue is resolved in your own editor, in git's own vocabulary**: each contested file opens with `diff3` markers relabelled by worker *and task* — `<<<<<<< w2 · add a per-tenant rate limit`, never an object id — with the base you both started from between the two sides. No bespoke merge pane: a coordination layer that needs a new visual language has been designed at the wrong altitude.
- **a conflict marker can never reach your working copy**: the gate re-reads what is actually about to be applied rather than what you were handed, so a file closed without saving is refused by name instead of landing markers in the repo.
- **a reconciled promotion credits both workers instead of invalidating one**: the journal entry carries every contributor, so neither derives `base moved` from work that is already inside it, and the entry says in words that neither was discarded and neither needs redoing. The peer's deliverable is closed out as `reconcile` — terminal, so its row settles; never approval, because nobody reviewed that deliverable on its own.
- **there is still exactly one promote path**: a reconciled change set is an ordinary frozen patch by the time git sees it, so `apply --check`, the parent-moved confirmation, and the journal all apply unchanged. Reconciling touches nothing until then — no worktree, no checkout, no index, no file written.
- **a semantic conflict can go back to a worker instead**: `Hand w2 · <task> both diffs to reconcile` sends both sides, the merge residue, your directive, and the provenance — that the other change is under review and not in the project — through the ordinary `tell` channel. The escape hatch, not the default: it costs a worker turn and returns an unreviewed deliverable, where a human edit is seconds and is reviewed by construction.
- **failure is a stated reason, never a quiet promotion of one side**: a git older than 2.38, a base Docket cannot reconstruct from either worker's head, an unmergeable pair — each degrades to the exits that already existed and says which one it hit.

- **the verdict card stops going quiet about a collision the dock is already showing**: the card graded its overlap from every peer's *raw* artifacts while the dock graded from the reviewed projection. A worker that has published names its files repo-relative in a frozen change set; the same worker's live `edit` calls name them absolutely, inside its own workspace. The two lists never intersected, so the card decided there was nothing to say — no `contested` line, no `o`. Both surfaces now read peers the same way: there is one notion of "affected" in this codebase.
- **a worker's workspace prefix is no longer part of the path it shares with anyone**: absolute tool paths are compared in the project's coordinate space, so a worker that is still running — the one whose overlap the human most needs to see — stops silently overlapping with nobody.
- **the delivery chip advances on its own, without ctrl+o**: the chip re-read its message only when the transcript rebuilt it, so `queued` sat there until the human toggled expand and the surface reported the past while claiming to be live. It is now painted from a fresh reading on each repaint, at most every 400 ms, and stops reading entirely once nothing it reports can change — a `read` message, a departed worker, or a tmux send that was never observable to begin with.

- **a blocked worker actually stops**: `docket_wait` and `docket_consult` used to *ask* the model to stop and then hope. A model that kept generating would restate the same blocker two or three more times in one turn, and every restatement became another question on another card — so answering one was followed by what looked like the same question coming back rephrased. Both tools now end the turn through pi's own abort, after the tool result resolves so the call and its answer stay in the transcript. Two calls in a single assistant message still both land; a second call after another model round-trip loses its turn, which is what it was told to do.
- **a backlog is answered in the order the worker got stuck**: replies bound to the *newest* open question, which resolved the restatement and left the question that actually blocked the worker for last. The card now answers the oldest open question, matching what the docs already described.
- **no surface hid that a backlog existed**: the verdict card marks which question it is answering and says `answering 1 of 3 · the rest stay open`, the dock row reads `3 questions` instead of `needs reply`, and answering one announces how many remain. A human who is told three are queued reads the second card as progress; one who is not reads it as a loop.
- **the wait tools say what stopping means**: end the turn, do not call again for the same blocker, and do not go hunting for the reply on disk — a real worker burned its turn running `find ../inbox` for an answer that arrives as a session message. A second call while already blocked says exactly what it cost: one more card for the human, and a later unblock for the worker.

- **you can now look at both sides of a collision**: a contested overlap puts one line on the verdict card and binds `o`, which opens the contested paths only, with each worker's hunks under a header naming that worker *and its task*. Offered only when another worker is actually contesting those lines, the same way `d` and `h` appear only when there is a change set to open.
- **the promote confirmation stops making you leave to find out**: `See both diffs` is one keypress from the modal and returns to it. Cancelling, hunting down the other worker, opening its card, and coming back was the friction that made the warning easy to dismiss.
- **the exit is a decision about who yields, not a merge**: closing the view asks one question with the safe answer as the default — send nothing, or ask one named worker to yield. Choosing a worker opens the revision editor and sends through the ordinary `tell` channel. It records no verdict, changes no lifecycle state, and merges nothing; Docket surfaces the collision and the workers still do the work.

- **overlap stops meaning "same path" and starts meaning "same lines"**: two workers editing different functions in one file used to warn exactly as loudly as two rewriting the same five lines, and that warning gated a modal — a warning that is usually wrong gets clicked through. Promotion now grades the overlap from patches Docket already froze: `same file`, `adjacent`, or `contested`. Both workers branched from the same base, so their patches' old-file line ranges are directly comparable; the grading is pure and unit-tested against fixture patches.
- **where it can be observed, it is observed rather than inferred**: when both workers have a frozen change set and the ranges are close enough to matter, git is asked whether the other worker's patch still applies once this one lands, by being handed both patches in order. If the first patch does not apply at all the base has already moved, and that reads as "could not tell" rather than as a conflict.
- **silence has to be earned**: the promote confirmation now appears unless Docket *observed* that the edits do not meet. A `contested` or `adjacent` overlap asks; so does one it could not grade, because the other worker is still working and has frozen nothing. Only a graded, separated overlap promotes quietly.
- **the confirmation predicts its own consequence**: it names the contested file, both workers' line ranges, the task text of the worker it collides with, whether that worker's change set survives, and the line that was always missing — `promoting this leaves w2 building on the old version`. That is a prediction of something P4 already makes true the moment the promotion lands.

- **the dock stops growing with the fleet**: a worker whose decision is behind it — `reviewed`, or `stopped` without ever making a claim — no longer holds a row. It folds into one line, `4 settled · f8`, and `f8` is where the whole roster still lives. Each surface now answers exactly one question: the dock "is anything waiting on me", `f8` "what is the whole fleet doing", `/docket` "what needs a decision". A message the worker never took un-settles its row, because there you already acted and nothing has happened yet.
- **the fold opens with the key that already means that**: the workers dashboard shows `+4 settled · tab to show` and `tab` reveals the band as its own group, the same vocabulary the broadcast picker uses for its hidden `unrelated` band. The key is offered only while something is folded. `tab` no longer means "next worker", which duplicated `j`/`down`.
- **a recorded verdict outranks its own warnings**: a settled row read `overlap w6: src/api/limit.ts` instead of `reviewed`, and could still carry `base moved`. Neither can change a decision already made, so the state wins the cell and the staleness line is suppressed — it returns on the verdict card if the worker publishes another generation, which is where it acts.
- **dock rows are columns, not a ragged line**: worker, state, task, detail, age, chip are fixed widths for a whole render, so separators line up down the screen. Under width pressure the detail column gives ground first, then the state word and the chip; task text and worker kind give ground last, because they are what identify a row. The task no longer truncates at 32 characters when the column is twice that wide, and an overlap warning on the dock names the file rather than a path cut mid-directory.
...
- **`/docket broadcast <text>` tells the workers a change affects, and Docket works out who**: every running worker is scored against paths the message names, paths it has touched, files its approved plan declared, identifiers, and task-word overlap, then sorted into affected / maybe / unrelated with the reason shown beside each row. You confirm; you never type a worker id.
- **no row is ever a bare index**: every candidate carries its task text and kind. A human coming back after twenty minutes does not remember which index holds which job, and a surface that assumes otherwise is a guessing game wearing the costume of control.
- **path matching survives worktrees**: basenames are compared alongside full paths, so a worker recording paths relative to its own isolated workspace still matches the file the message names.
- **a broadcast does not interrupt**: it is delivered `nextTurn`, entering each worker's context without triggering a turn. A worker mid-edit keeps working; a worker blocked on a question stays blocked on it. Broadcasts never redirect a worker and never answer something it is waiting on, and several pending ones arrive as one block.
- **standing rides with the claim**: a forwarded worker claim carries `approved · promoted`, `in worktree, not promoted`, or `notice, unreviewed`. Content is never restricted — "code is ready" is a legitimate broadcast — but a receiving worker can tell landed code from another worker's workspace.
- **uncertainty proposes the bulletin, not an empty grid**: when nothing scores as affected, the primary action becomes posting to the standing project note that every worker re-reads before its first edit and at each plan gate. It lives at `~/.pi/agent/docket/bulletins/<project>.md`, outside the repo, so every worktree reads the same current file rather than a snapshot from when it started.
- **a notice is one keypress from a broadcast**: opening a notice in the docket offers Broadcast as its primary action, with any workers the notice addressed preselected and tagged. The worker expresses intent; you keep the decision, and workers still cannot reach each other directly.
- **companions can watch the messaging layer without owning it**: `globalThis.__docket.onMessage(handler)` observes messages in either direction. It carries **no body** — a dashboard needs to know that w1 was told something and whether it was taken, not the text, and handing bodies to every subscriber would make Docket's metadata-only discipline meaningless at the seam. Observations decode from the same emission that feeds `onWorkerEvent`, so an observer can never see a message the event stream did not.
- **`registerBroadcastAdvisor` proposes, and can do nothing else**: a companion may surface a recipient Docket's evidence missed. A suggestion lifts a candidate from `unrelated` to `maybe` and never to `affected` — affected is preselected and Enter sends it, so an advisor able to reach that band would be an advisor able to cause a delivery. It cannot demote, deselect, remove, or name a worker Docket did not already enumerate. Suggested rows read `suggested · <reason>`.
- **advisors are isolated like every other subscriber**: additive rather than exclusive, given a 250 ms window, and dropped on throw, hang, or malformed output. The permission line, attribution, and the human keypress before any broadcast are not extension points.
- **a worker that is gone says so, whatever its work amounts to**: Docket kept one fact where it needed two — what the work amounts to, and whether a process is still there. A worker that finished and was then quit came back as plain `ready` with a live dot, offering to be told, steered, and stopped, and the only surface that disagreed was a message sub-line. Liveness is now observed separately from state: a hollow `○` replaces the filled dot everywhere, the verdict card reads `ready · not running`, and the dock row reads `ready · gone`. Nothing about the deliverable changes — it is still ready, and still reviewable.
- **a stopped process is no longer reported as a finished worker**: `ended` used to become `ready` whenever the worker had accumulated any artifacts, and artifacts accumulate from the first file a worker reads. Only `docket_done` earns `ready` now; a process that merely stopped derives `stopped`, which is openable — its evidence and diff stay reachable — but never coloured, never chipped, and never presented with the task echoed back under an `Outcome` heading. Its verdict card offers Respawn, Dismiss, and Discard, because there is no claim to accept.
- **an exit no longer erases what a worker reported**: session end and the exit trap now record `exitedAt` as its own fact instead of overwriting `state`, so a reported outcome survives the session that produced it. A heartbeat retires the observation; a respawn clears it.
- **an undeliverable message says so on the chip you were looking at**: `tell` to a worker that is not running reads `undeliverable · worker is not running` and turns amber in place, with `/docket respawn w<N> delivers it` beneath it — instead of sitting at `queued` while a dock sub-line you had to go and find said otherwise. The message is still on disk, and still delivers on respawn.
- **an escalated consult says each thing once**: the reason the parent agent declined appeared three times — in the in-flight call line, in the expanded card, and again in the notification. The call line now names the act (`handing w3's question to you`), the collapsed result carries the *question* you have to answer, and the reason, the full question, and `f8 → w3 → Enter` live behind the expand control. The toast fires only when the sweep escalated with no card to point at.
- **a promotion tells the workers it invalidates, without you addressing it to anyone**: approving a worker's changes appends them to the project journal with the files that landed. Every worker that read, edited, or has an approved plan naming one of those files shows `base moved · N files it works on landed since it started` — muted, because nobody is blocked. This is the one worker output that already carries your signature, so propagating it needs no second confirmation, no recipient picker, and no model tokens.
- **a stale deliverable says so where it matters**: a worker that finished before those files landed carries `produced before <paths> landed · re-check before promoting` on its verdict card, in warning colour. That is the moment the fact bears on — approving a diff against a base that no longer exists is the mistake it prevents.
- **nothing is pushed, and nothing is woken**: a worker mid-turn stays mid-turn. Staleness reaches it through the journal at a gate it was already going to stop at, and its guardrails now state plainly that its isolated workspace still holds the version from *before* the change landed — so it reports the assumption instead of silently rebuilding on a stale copy.
- **one notion of "affected"**: `deriveStaleBase` routes through the same scorer a broadcast uses. Only `affected` counts — task-text overlap is a fine reason to propose a recipient to you, and a poor reason to tell a worker its ground has shifted.
- **the bulletin becomes the project journal**: `bulletins/<project>.ndjson` is the append-only source; the markdown workers read is regenerated from it in full on every append, so the two cannot drift and editing it changes nothing. An existing bulletin migrates on first read. The path a worker is pointed at is unchanged, so a worker started before the upgrade keeps reading a file that is still being written.
- **messages read as correspondence**: a sent message is headed `✉ docket · message` and its line reads `→ w2 · ✓✓ read · 8:36 PM` — direction, correspondent, delivery, time, in the vocabulary every messaging client already taught you. One tick queued, two delivered, two read, `⚠` undeliverable. The tick never replaces the word; both always travel, because a tick nobody can decode is worse than a word nobody shortened.
- **rationale**: see [docs/adr/0008-worker-messaging.md](docs/adr/0008-worker-messaging.md) for the permission line, the consult lane, and the broadcast design.
