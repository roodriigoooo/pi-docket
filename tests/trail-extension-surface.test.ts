...
import { installTrailExtensionSurface, getTrailExtensionSurface } from "../extensions/trail-extension-surface.js";

test("installTrailExtensionSurface exposes globalThis.__trail", () => {
	const registry = createWorkerKindRegistry();
	const surface = installTrailExtensionSurface(registry);
	assert.equal(getTrailExtensionSurface(), surface);
